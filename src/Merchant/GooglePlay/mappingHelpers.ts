...
function descriptionHtmlLocalized (searchArray: any[]) {
...
function priceText (priceText: string) {
  return priceText || 'Free';
}

function normalizeAndroidVersion (androidVersionText: string) {
...
function buildHistogram (container: any[][]) {
  if (!container) {
...
function extractFeatures (featuresArray: never[][] | null) {
 ...
 */
function extractCategories (searchArray: any[] | null, categories = []) {
